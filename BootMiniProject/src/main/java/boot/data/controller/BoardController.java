...
public class BoardController {
	 ...
	 * 
	 * mview.setViewName("/board/boardlist"); return mview; }
	 */
	...
	public String insert(@ModelAttribute BoardDto dto, HttpSession session)
	{
		...
		return "redirect:content?num="+ maxnum; //일단은 list... content로 이동하기 
		//return "redirect:content?num="+ service.getMaxNum(); //가능
			...
			@RequestParam(defaultValue = "1") int currentPage)
	{
		...
		String ext= dto.getUploadfile().substring(dotLoc+1); //현재위치 다음부터 끝까지 // . 의 다음글자부터 끝까지 추출 (.은 포함되면 안되기때문)
		...
		if(ext.equalsIgnoreCase("jpg") || ext.equalsIgnoreCase("gif") || ext.equalsIgnoreCase("png") || ext.equalsIgnoreCase("jpeg"))
			...
			mview.addObject("bupload", false); // 이미지 인지 아닌지를 보고 출력하기 위해서
